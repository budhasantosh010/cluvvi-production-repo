...
const cluvviHome = resolve(root, ".cluvvi-test", `browser-structured-${process.pid}`);
const fixtureProject = resolve(
  root,
  "tests",
  "fixtures",
  `.browser-structured-fixture-${process.pid}`,
);
...
async function waitForHealthy(worker, web) {
  ...
  while (Date.now() < deadline) {
    if (worker.exitCode !== null)
      throw new Error(`Structured worker exited with ${worker.exitCode}.`);
    if (web.exitCode !== null)
      throw new Error(`Structured web server exited with ${web.exitCode}.`);
    try {
  ...
  throw new Error("Structured browser environment did not become healthy.");
}
...
const environment = {
  ...
  CLUVVI_FIXTURE_STAGE_DELAY_MS: "250",
...
try {
  ...
  tests = spawnChild(
    ...
    [
      ...
      "test",
      "--config",
      "playwright.structured-content.config.ts",
      "--reporter",
      "list",
    ],
    environment,
