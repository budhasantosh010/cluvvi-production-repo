# Cluvvi C1-J.3 Local Architecture
...
C1-J keeps source adapters in Project A and keeps Project B as an independent consumer. Project B never imports Project A source modules. With `selected_sources`, the process bridge may emit C1-J.1 hiring companions, the C1-J.2 Reddit community family, and/or the C1-J.3 GitHub developer family. The developer family consists of `developer_source_plan.v1`, `developer_repository_collection.v1`, `developer_thread_manifest.v1`, `developer_thread_metadata.v1`, `developer_comment_collection_manifest.v1`, `developer_comment_metadata.v1`, `developer_signals.v1`, `developer_source_run_telemetry.v1`, and manifest-referenced universal `thread.v1` / `comment_collection.v1` files.

The Cluvvi engine validates source-family artifacts independently and persists community work in seven separately resumable stages and developer work in eight separately resumable stages, each with a dedicated configuration fingerprint. A broken later community or developer artifact can therefore be repaired and resumed without rerunning valid discovery, extraction, structured parsing, hiring, or earlier source-family stages.

Public Reddit and GitHub text enters Evidence only as `untrusted_public_content`; entity attachment requires a conservative unique match to an already-known discovery entity. Reddit usernames, GitHub usernames, display names, author associations, repository owners, and comment authors never become buyer/contact identities. Normal ranking components ignore community and developer signals. Repeated qualifying community evidence may contribute at most one separate point, and qualifying developer evidence may contribute at most one separate point only when it spans at least two independent repositories. Buyer Map preserves source-specific thread/comment/repository/release/signal provenance and explicit evidence limitations.

Secrets remain owned by Project A. Optional SmartRecruiters and GitHub credentials are never forwarded, serialized, fingerprinted, logged, or rendered by Cluvvi. Reddit is keyless. GitHub is public-only from Project B's perspective; optional authenticated-free access can occur only inside Project A and never widens the private-data boundary. Cluvvi does not add Reddit OAuth/login/cookies/paid APIs/challenge bypass, private GitHub repositories, cloning, source/diff/patch or release-asset downloads, GitHub mutations/workflows, candidate/application data, private ATS endpoints, authenticated/private community access, contact enrichment, monitoring, or outreach.
